...
using System.Text;
using Product = Cosmetic_Web.Models.Product;
...
namespace Cosmetic_Web.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Details(int id)
        {
            ...
            .AnyAsync(o => o.OrderDetails.Any(od => od.ProductId == id && o.Status.Equals("Đã nhận hàng") && o.UserId == userId));
            bool userHasRated = ls.Any(p => p.Comments.Any(c => c.UserId == userId));
        ...
        [HttpPost]
        public async Task<IActionResult> AddtoCart(AddToCartViewModel model, int Quantity)
        {
            var existingCartItem = await _context.CartDetails.Where(u=>u.UserId == model.UserId).FirstOrDefaultAsync(cd => cd.ProductId == model.ProductId);
        ...
        public async Task<IActionResult> GetProductsByCategory(int categoryId, string? searchKeyword, string? sortOption, int pageNumber = 1, int pageSize = 12)
        {
            ...
            {
                productsQuery = productsQuery.Where(p => EF.Functions.Like(p.ProductName, $"%{searchKeyword}%") || EF.Functions.Like(p.Description, $"%{searchKeyword}%"));
            }
